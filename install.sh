...
install_v2ray() {
	...
	if [[ $local_install ]]; then
		...
		pushd /tmp
		if [[ $_test ]]; then
			git clone https://github.com/yjulm/233boy_v2ray_v3.05 -b test /etc/v2ray/233boy/v2ray
		else
			git clone https://github.com/yjulm/233boy_v2ray_v3.05 -b v3.05-fix /etc/v2ray/233boy/v2ray
		fi
		popd
...
install() {
	if [[ -f /usr/local/bin/v2ray/v2ray && -f /usr/local/etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/233boy/v2ray ]]; then
		echo
		...
		echo
		exit 1
	elif [[ -f /usr/local/bin/v2ray/v2ray && -f /usr/local/etc/v2ray/config.json ]] && [[ -f /etc/v2ray/233blog_v2ray_backup.txt && -d /etc/v2ray/233boy/v2ray ]]; then
		echo
